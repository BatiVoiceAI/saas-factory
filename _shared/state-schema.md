...
- **archetype** : web-saas | landing | automation | **ecommerce**   (défaut `web-saas`) — forme **TECHNIQUE** du livrable ; **conditionne le socle de complétude (07)**. **Châssis A-à-Z clé-en-main : `web-saas` + `automation`** ; **`ecommerce` = logique commerce LIVRÉE** (`_shared/blocks/ecommerce/` : SQL + RPC + pièges P1/P2/P3, `verify:machine` + tests verts) dont l'**UI se dérive du châssis web-saas** (remplace `billing` par le checkout one-shot). `landing` = **assemblage clé-en-main** du sous-ensemble web-saas (skeleton + `components/landing/*` + `waitlist` + notifications + légal — tous LIVRÉS, `tsc`-verts) et `tenancy=multi-org` = **substrat `org-tenancy` LIVRÉ** (bloc web-saas : `0006_org_tenancy.sql` + `lib/org/*` + org-switcher, RLS par org, `tsc`-vert). **Les 4 archétypes + le substrat org sont couverts**, la **calibration amont** d'un `landing` (02/03/05 allégés) **implémentée** (routing §Archétype landing + conditionnement `archetype=landing` dans 02/03/05). Seul résidu **universel** : l'épreuve run (build SSR + E2E) à la 1re instanciation, comme tout `web-saas` (`_shared/blocks/web-saas/MOAT-STATUS.md`) — `skills/saas-factory/references/routing.md` + `CONTRIBUTING.md`.
...
- **`landing` → assemblage clé-en-main LIVRÉ** : skeleton (avec `components/landing/*`) + **`waitlist`** + notifications + légal, tous livrés (`tsc`-verts). Le `landing` s'assemble par **sélection du sous-ensemble web-saas** (pas de bloc séparé, par conception) ; câblage : `skills/11-project-setup/references/scaffold-procedure.md` §Sélection. **Reste** : calibration amont 02/03/05 (allégée à la main).
- **substrat `org-tenancy` → LIVRÉ** (bloc web-saas) : `0006_org_tenancy.sql` (`orgs`/`org_members`/`org_invitations` + RLS par org + helpers `security definer` lesson #15) + `lib/org/*` + `components/org/org-switcher.tsx`, `tsc`-vert ; `org-billing` = extension typée (stub documenté).
