...
## Châssis `ecommerce` — boutique en ligne *(archétype `ecommerce`, LIVRÉ)*

✅ **Livré (logique commerce + SQL + pièges P1/P2/P3 ; `tsc` + `verify:machine` (5 lints) + tests `node:test` verts).** L'archétype **`ecommerce`** est de **première classe** (`../archetypes/ecommerce.md`) et son châssis **`_shared/blocks/ecommerce/`** **existe** : châssis **séparé et dependency-light** (comme `automation/`) qui livre la **LOGIQUE commerce testée** — `lib/pricing` (P2, recalcul serveur) · `lib/inventory` (P1, décrément atomique) · `lib/cart` / `lib/orders` · `webhook` (P3, idempotent) — **+** les **migrations SQL** (schéma / RLS + RPC `fulfill_paid_order` qui réunit P1+P2+P3 en une transaction) **+** `verify:machine` (5 lints) **+** tests `node:test` **verts**. Sélectionné par l'étape 9 **uniquement** quand `archetype=ecommerce` (`_shared/state-schema.md` §modèle 3 axes) ; **ne jamais** forcer l'assemblage `landing`/`automation` dessus. Manifeste bloc → fichiers réels : `ecommerce/BLOCKS.md`.

**L'UI/app se dérive du châssis `web-saas`.** Le châssis `ecommerce` fournit la **logique** (ci-dessus) ; la **vitrine + panier + checkout + back-office + route webhook Next** se **dérivent du socle web-saas** (le modèle « socle léger + blocs codés par projet ») : on **réutilise** `skeleton` + `ui-shell` + `auth` *(compte client léger et **optionnel** — le **checkout invité** est le défaut e-commerce)* + `crud` (produits/commandes) + `notifications` (confirmations) + légal + `repo-ci` + `hosting`, et on **remplace le bloc `billing` (abonnement Stripe) par le checkout Stripe ONE-SHOT** (`mode:payment`, *jamais* `mode:subscription` — c'est la différence dure avec `billing`). Cette dérivation UI est **documentée dans `ecommerce/BLOCKS.md`, pas compilée** dans ce châssis. Le CTO (09) architecture, le DEV (12) câble la logique livrée à l'UI dérivée.
