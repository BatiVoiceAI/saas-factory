...
| `auth` | Passwordless OTP + magic link (Server Actions `lib/auth/actions.ts`), callback/signout routes, `auth-form`, `get-user`, **enrollment par type** (`lib/auth/enrollment.ts` : invitations / allowlist domaine / compte seedé), page `(app)/dashboard`, migration `0001_auth.sql` (profiles + rôles + trigger profil) | RLS activée ; anti-escalade `role` ; enrollment restreint (autorité = `disable_signup` Supabase) ; redirections via `NEXT_PUBLIC_SITE_URL` (safe derrière proxy) |
