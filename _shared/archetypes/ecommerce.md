...
> **Scaffold code : châssis `../blocks/ecommerce/` (à bâtir — parité avec `automation/`).** Ce fichier pose le **modèle** (forme du livrable, socle EC1-EC5, patterns d'architecture, pièges) ; le châssis qui scaffolde une boutique (catalogue + panier + checkout one-shot + commandes + stock) est un bloc partagé **`_shared/blocks/ecommerce/`**. En attendant le châssis assemblé, une boutique se **build en blocs custom** autour du socle web-saas (le modèle « socle léger + blocs codés par projet ») : le CTO (09) architecture les patterns ci-dessous, le DEV (12) code les blocs. **La détection (01) + le socle (07) + le routage + l'architecture sont définis ; c'est un archétype de PREMIÈRE CLASSE, pas un cas web-saas déguisé.**
...
## Structure du châssis généré → `../blocks/ecommerce/` (à bâtir)
```
app/
