...
- [ ] **Events du funnel émettent en staging** (bloquant) : `user_signed_up` + `activation_completed` (le moment magique du PRD) sont **visibles dans PostHog live-events** après un parcours de test — un funnel muet au ship rend la Phase 6 aveugle dès le premier tour. **Conditionné par archétype** : web-saas (public/interne/perso) = funnel ci-dessus ; `landing` → `waitlist_joined` ; `automation` → **n/a funnel** (headless), on vérifie à la place que les **métriques de run** remontent (AU2 `run_log` alimenté + AU3 healthcheck) — `automation-deploy.md`.
