...
Un headless **n'a pas d'auth utilisateur ni de RLS produit** → la **recette live AUTHENTIFIÉE (17b)** est **sans objet** ; on ne « franchit » aucune session. Elle est **remplacée** par une **vérification de boucle fermée headless** (règle 7 de `skills/phase-5-launch/references/routing.md`, colonne `automation`). Ce n'est **pas** une recette d'auth — c'est la preuve que **le job produit sa valeur et se supervise lui-même**.

> **Trois formes sœurs du contrôle 17b** (une par archétype, **même rigueur** — preuve réelle sur la prod, budget **3 cycles**, jamais de « livré » sur un faux vert) : **web-saas** = recette **AUTHENTIFIÉE** (`live-qa.md`) ; **ecommerce** = **achat de test réel A→Z** en Stripe mode test (`live-qa.md §Variante ECOMMERCE`) ; **automation** = cette **boucle fermée headless**. Le **rejeu qui prouve l'idempotence** est commun aux trois — ici le **double-déclenchement `workflow_dispatch`** (0 double effet), en ecommerce le **rejeu du webhook Stripe** (0 commande en double).

Sur le scheduler réel (ou via `workflow_dispatch`), prouver la chaîne **run → logs → boucle → idempotence** :
