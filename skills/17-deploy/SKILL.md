...
1. **Pré-vol** (bloquant) — build + tests **verts**, le **livret** (`qa/test-booklet.md`) **sans `FAIL` bloquant**, secrets en **env** (jamais en dur), migrations prod prêtes (idempotentes), rollback testé, **services tiers & déclencheurs soldés** (décisions « déférées » résolues, scheduler branché + exécution prouvée, email réel parti, confirmation réactivée, redirect URLs prod, events du funnel émis en staging). → `preflight-checklist.md`.
...
3. **🚪 PORTE de publication** (`AskUserQuestion`) — **OK explicite** avant tout apply. (Au-delà du « ship » produit de l'étape 15, c'est le feu vert **technique** au cutover.) La **même** question valide le **Critère de KILL** proposé (`{métrique live + seuil + fenêtre}`), écrit dans `state.md` **avant** de voir les chiffres. **Jamais** de publication sans réponse. → `publication-gate.md`.
4. **Apply** (via MCP infra, ordre strict) — migrations prod → **promotion staging→production** (Vercel/CF) → **cutover DNS** (Cloudflare) → **activation tracking** (PostHog + Sentry) → **filet post-launch** (alert rule Sentry sur le parcours cœur, backups actés dans `deploy/log.md`, uptime monitor si public). Chaque sous-étape d'apply = un point de rollback.
5. **Health check (canary)** — après déploiement : pages clés à 200, parcours cœur OK en prod **avec réception réelle de l'email de connexion**, pas de pic d'erreurs Sentry, Core Web Vitals OK. **Échec → rollback** — re-promotion N-1 si une version prod existe, sinon (**premier ship**) **dépublication → preview URL privée** — + log, pas de faux succès. Seuils chiffrés (source unique) + machine à états → `canary-rollback.md`.
...
## Sortie & état
Produit **en ligne** + tracking actif + santé vérifiée + **recette live passée** (`deploy/live-qa-report.md`, 0 bloquant, boucles fermées prouvées) + **filet post-launch posé** (alert rule Sentry cœur, backups actés, uptime si public) + **Critère de KILL écrit** dans `state.md` (`{métrique live + seuil + fenêtre}`, validé à la porte — c'est ce que 19-retro confrontera aux chiffres). Mets à jour `.saas-factory/state.md` (déployé, URL, version, `recette_live`, `Critère de KILL`). Résume (URL live + santé + verdict recette + correctifs appliqués). **Fin de Phase 5** → Phase 6 (mesure & retro).
