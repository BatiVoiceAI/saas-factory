...
**Grille de décision — moteur `startup-design` Ph8 « Validation ».** Le verdict, et surtout les **kill-criteria**, dérivent de la **Phase 8 (Validation)** du moteur vendoré : lis `{PLUGIN_ROOT}/vendor/startup-skill/startup-design/SKILL.md` §Phase 8 (idea **scorecard** 7 dimensions + **kill-criteria** mesurables + assumptions-tracker). On en **calibre** deux choses pour la porte : (a) le **scorecard** enrichit la cotation des axes (§4, sans la remplacer) ; (b) les **kill-criteria pré-enregistrés** (5-7 conditions d'arrêt SPÉCIFIQUES + MESURABLES, chacune liée à un seuil ou une expérience — « si < 3/10 interviewés paieraient X », pas « si pas de demande ») sont **écrits dans l'`opportunity-brief`** pour que **19-retro les relève** plutôt que de réinventer un seuil a posteriori. Carte du pilotage moteur : `_shared/startup-engine-map.md`.
...
|---|---|
| `research/idea-brief.md` (problème, cible, type/route) | `research/opportunity-brief.md` — **le SEUL livrable de décision** (template `assets/templates/opportunity-brief.md`) : bloc §Décision (POURQUOI, non-tech) en tête **+** le détaillé traçable **+ §kill-criteria pré-enregistrés** (mesurables, dérivés de la Ph8 « Validation » — la grille que **19-retro** relèvera) |
...
> **Option — pitch investisseur (sur demande, HORS pipeline standard).** Une usine à micro-SaaS **ne pitche pas par défaut**. Mais si le fondateur veut lever / présenter (demo-day, accélérateur, investisseur), après un **Go** tu peux **proposer** de générer un **pitch multi-format** (10/5/2/1-min + email + Q&A + scoring) via le moteur vendoré `{PLUGIN_ROOT}/vendor/startup-skill/startup-pitch/` : il **consomme** les artefacts déjà produits (idea-brief, market, positioning, business-model, scorecard) et **saute son intake**, dans la **langue** de l'utilisateur. Purement **opt-in** — ne l'impose jamais, ne bloque jamais la porte dessus. (Le one-liner/elevator pour la landing est déjà couvert par `startup-positioning` en 03 ; pitch n'ajoute que le **narratif investisseur**.) Cf. `_shared/startup-engine-map.md`.
