...
2. **CTO** (`agents/cto.md`) — archi + **sécurité** (moteur `{PLUGIN_ROOT}/vendor/security-review/` exécuté sur le diff + `references/owasp-cards.md` : OWASP/STRIDE/LLM/Agentic) + fonctionnel.
3. **Designer** (`agents/designer.md`) — conformité `DESIGN.md` + UX + **accessibilité** (moteur `{PLUGIN_ROOT}/vendor/accessibility-review/SKILL.md` exécuté) + **checklist anti-slop/convergence** de `_shared/design-doctrine.md` (binaire, point par point, **19 points** — dont **rationale par page tenu** (18) et **`prefers-reduced-motion`** (19) ; **un marqueur coché = `FAIL`** → retour dev). **→ `N/A` si `archetype=automation`** (headless, pas de surface : verdict N/A tracé, l'edge passe au CEO-persona — `references/cascade-protocol.md` §Conditionnement, `verdict-schema.md`).
