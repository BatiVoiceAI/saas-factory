...
> **`ecommerce` → cran sécurité RENFORCÉ (3 pièges non négociables).** Pour toute feature de commerce (panier, checkout, paiement, stock), le cran CTO **exige la preuve** des 3 pièges de `_shared/archetypes/ecommerce.md` §Pièges, traités comme findings sécu de premier plan : **P1** décrément stock **atomique** (aucune fenêtre de course `SELECT`→`INSERT`), **P2** total **recalculé serveur** (jamais un total client ; un panier au prix altéré est rejeté/recalculé — faille d'**intégrité**), **P3** webhook **idempotent** (commande créée une seule fois sur redelivery — `unique(stripe_session_id)` + signature vérifiée). **Un seul de P1/P2/P3 non prouvé = `FAIL`** (au même titre que l'idempotence 2-grains exigée pour `automation`). Détail playbook : `reviewer-playbooks.md` Cran 2.

## Cran 3 — Designer (`agents/designer.md`)
> **`automation` (headless) → cran N/A** (verdict `N/A` tracé, la feature monte au CEO-persona qui porte l'edge boucle fermée + idempotence) — jamais un `FAIL` pour absence de surface. `web-saas` / `landing` / **`ecommerce`** → cran complet ci-dessous (une boutique **a** une UI — vitrine, fiche produit, panier, checkout ; `ecommerce` ne reçoit **jamais** un `N/A` Designer). Cf. `cascade-protocol.md` §Conditionnement + `agents/designer.md`.
