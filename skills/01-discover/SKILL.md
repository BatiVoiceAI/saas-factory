...
**✅ Périmètre : on DÉTECTE et on NOMME l'archétype/tenancy — et tous sont buildables.** Châssis `automation` + `ecommerce` (logique) LIVRÉS ; `landing` s'assemble clé-en-main du sous-ensemble web-saas ; substrat `org-tenancy` LIVRÉ (bloc web-saas). Le nommer ici cadre l'aval sur du concret (reste, pour un `landing`, la calibration amont 02/03/05).
