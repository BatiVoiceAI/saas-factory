...
Lis `archetype` dans `.saas-factory/state.md`. Le funnel AARRR/PostHog ci-dessous suppose une **surface web avec un workflow d'activation** — vrai pour `web-saas` et `landing`, **faux pour `automation`** (headless, pas de PostHog activé à l'étape 17 : `skills/17-deploy/references/automation-deploy.md` retire les events funnel du pré-vol) **et de forme différente pour `ecommerce`** (la surface web existe, mais le funnel est un funnel d'**achat**, pas d'activation). N'essaie pas de lire un funnel qui n'existe pas (`automation`) ni le **mauvais** funnel (l'activation SaaS pour une boutique) — ce serait le symétrique du « funnel muet ».
...
- **`ecommerce`** → **funnel d'ACHAT**, pas d'AARRR d'activation. Source = **funnel PostHog** (`view_product → add_to_cart → begin_checkout → purchase`, câblés au call-site — EC1→EC4) **+ Stripe** (revenu / AOV, source de vérité du paiement). On lit : **taux de conversion** (visiteur → acheteur), **panier moyen (AOV)**, **taux d'abandon de panier** (fuite dominante = marche `begin_checkout → purchase`), **ré-achat / clients récurrents**, **revenu**. **Ni « moment magique » ni « taux d'activation »** — la « rétention » e-commerce = le **ré-achat**. Exige l'instrumentation `capture()` **au call-site réel** (fiche produit, bouton panier, page checkout, webhook de commande confirmée **avec montant**) : un funnel déclaré mais non câblé est **muet**. Détail complet : `references/metrics-framework.md` §Variante ECOMMERCE. Sentry/santé inchangé.
...
## Le moteur (on dépend)
- **PostHog** (activation, funnel, rétention) — déjà activé à l'étape 17 pour `web-saas` / `landing` **et `ecommerce`** (funnel d'**achat** ; **+ Stripe** pour revenu/AOV) ; pour `automation`, la source est `run_log` (AU2) + healthcheck (AU3), pas PostHog (cf. Garde archétype).
