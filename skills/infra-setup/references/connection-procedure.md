...
- **Méthode** : A (OAuth/MCP `cloudflare`) **OU** B (**API token** en `.env`). **B est le FALLBACK OBLIGATOIRE dès qu'aucun MCP Cloudflare n'est connecté** — on ne **saute JAMAIS** Cloudflare faute de MCP (c'est du **socle** : sans DNS, pas de domaine custom). Token B = un **Cloudflare API token** scopé **Zone → DNS → Edit** *(+ **Zone → Zone → Read** pour lister)* sur la/les zone(s), déposé sous `CLOUDFLARE_API_TOKEN` dans `~/.saas-factory/.env`, appelé en **API directe** (`https://api.cloudflare.com/client/v4`) — même discipline que GitHub/Supabase (jamais de valeur en chat). Le cas le plus courant en session non-interactive/headless = **B** ([[saas-factory-run-straula-landing]]).
