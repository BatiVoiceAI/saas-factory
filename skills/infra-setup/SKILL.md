...
Pour chaque outil : privilégie la **connexion via le MCP officiel de l'éditeur** — auth **OAuth** pour les serveurs hébergés (Cloudflare, Vercel, Stripe), **token déposé en env** pour les serveurs locaux (GitHub, Supabase) ; dans les deux cas la clé n'est **jamais collée dans la conversation**. Si aucun MCP officiel (Resend, LLM, obs), l'utilisateur **dépose lui-même** sa clé dans `~/.saas-factory/.env` (ou gestionnaire de secrets).
> **🚨 Si le MCP hébergé n'est PAS connecté (Cloudflare / Vercel / Stripe) → NE SAUTE PAS l'outil.** Bascule sur un **API token scopé déposé en `.env`** (appel **API directe**), exactement comme GitHub/Supabase. C'est le mode **par défaut en session non-interactive** ([[saas-factory-run-straula-landing]] : « provisioning = direct-API avec .env tokens, pas MCP »). **Ne jamais laisser un provider du socle (surtout Cloudflare) hors de la config faute de MCP** — sinon un run passe sur Supabase/Resend/GitHub mais oublie le DNS, et le domaine custom ne se câble jamais. **Ne provisionne rien ici** : on ne fait que connecter et stocker. Pour chaque outil : **connecter → sonde → écrire `config.json`** ; un `connected` sans sonde réussie est **interdit**.
...
1. **GitHub** (repo + CI) — connecte via le MCP `github` (**token PAT** déposé en `.env`, pas OAuth). Sert à créer + pousser un repo par projet + GitHub Actions **+ demande l'email qui authore les commits** (celui du compte GitHub) → stocké en `config.git_author` (`{name, email}`, non-secret). Défaut **proposé** = l'email du compte, mais **overridable**. Cet email **DEVRA être membre de la team Vercel** (cross-check au step 4), sinon le déploiement sera **BLOCKED**. *[socle — forcing git-author : `connection-procedure.md §1`]*
...
3. **Cloudflare** (DNS + domaine) — **socle, NE JAMAIS sauter** (sans lui, aucun projet n'a de domaine custom — `[[saas-factory-toujours-brancher-domaine]]`). Connecte via le **MCP `cloudflare` (OAuth)** s'il est disponible ; **SINON — fallback OBLIGATOIRE, pas optionnel** : dépose un **API token Cloudflare scopé** (`CLOUDFLARE_API_TOKEN`, permissions **Zone → DNS → Edit** + **Zone → Zone → Read** sur la/les zone(s)) dans `~/.saas-factory/.env`, appel **API directe** (`api.cloudflare.com`). **+ demande le nom de domaine principal** (pour les sous-domaines `projet.tondomaine.com`). *[socle — forcing domaine + fallback token : `connection-procedure.md §3`]*
4. **Hébergement** — choix : défaut **Vercel** ; alternatives **Cloudflare Pages/Workers** ou **Coolify** (self-host, 1 VPS pour tout). Connecte le provider retenu **+ cross-check git-author** : après connexion Vercel, vérifie que `config.git_author.email` (capturé au step 1) est bien **membre de la team Vercel** (lisible via l'API Vercel) ; sinon **avertis que le déploiement sera BLOCKED** et re-demande l'email (ou invite à l'ajouter à la team). *[socle — matrice de choix : `decision-matrices.md` · cross-check team : `connection-procedure.md §4`]*
