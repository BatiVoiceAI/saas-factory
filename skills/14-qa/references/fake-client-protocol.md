...
> 🚨 **Le Parcours #0 existe pour les quatre archétypes, mais SA FORME dépend de l'`archetype`** (`state.md`) — parce que le **socle de complétude est lui-même conditionné par archétype** (🚨 SOURCE : `_shared/state-schema.md` §« le socle est CONDITIONNÉ PAR ARCHÉTYPE » ; portes 14 par archétype : `skills/saas-factory/references/routing.md`, ligne `14-qa`). Un `landing` n'a **ni signup, ni onboarding, ni dashboard** ; un `automation` est **headless** (pas d'UI produit) ; un `ecommerce` n'a **pas de signup obligatoire** (le **checkout invité** est le défaut) **ni de dashboard SaaS** (son back-office admin n'est PAS un dashboard applicatif). **Ne JAMAIS recaler un `landing`/`automation`/`ecommerce` au motif « pas d'onboarding/dashboard »** : c'est le faux-négatif que ce modèle corrige (au même titre que « légal FR en dur » ; exiger le socle web-saas d'une boutique = faux-négatif d'archétype — `_shared/archetypes/ecommerce.md`). Le **calibrage par type** (public/interne/perso) s'ajoute **par-dessus** l'archétype — il ne le remplace pas. Joue **la branche de l'archétype du run**, pas les quatre.

Point commun aux quatre : **on commence là où le vrai utilisateur commence, aucun raccourci** (pas d'URL interne, pas d'état pré-fabriqué, pas de déclencheur contourné, pas de webhook de paiement appelé à la main) — le but est de **vivre** l'arrivée réelle, pas de la supposer.
...
##### D · `ecommerce` — le parcours d'achat (catalogue → panier → checkout invité → paiement → commande/stock/boucle fermée)
Le scénario commence à l'URL publique de la **boutique**, **sans compte** — le **checkout invité** est le défaut e-commerce (l'auth client est **optionnelle**, jamais un prérequis à l'achat — `_shared/archetypes/ecommerce.md`). **Aucun raccourci** : pas d'URL interne, pas de commande pré-créée, **pas de webhook de paiement appelé à la main** (on paie pour de vrai, en mode test). **Ne recale JAMAIS une boutique au motif « pas d'onboarding/dashboard SaaS »** — le back-office admin n'est PAS un dashboard applicatif. Ce qu'on prouve (parcours nominal A→Z, dans un **vrai navigateur**) :

1. **Catalogue** — parcours la vitrine : liste produits (nom, **prix**, images réelles, disponibilité), ouvre une **fiche produit**. Un produit **non publié** ne doit **pas** être visible (RLS : `select` anon sur les produits publiés seulement). Placeholder/lorem/prix absent = `FAIL`.
2. **Panier** — ajoute au panier, **modifie la quantité**, retire un article : le **sous-total est recalculé côté serveur** à chaque changement (jamais un total envoyé par le client — cf. P2). Panier vide → état vide avec CTA « continuer mes achats ».
3. **Checkout invité + paiement (carte de test Stripe)** — passe au checkout **en invité** (email suffit), paie avec la **carte de test Stripe `4242 4242 4242 4242`** (date d'expiration future, CVC + code postal quelconques). Le montant débité est **celui recalculé serveur depuis le catalogue**, jamais un total client. Le checkout doit être un **paiement one-shot** (`mode:payment`) : un `mode:subscription` = `FAIL` (une boutique ne vend pas un abonnement — c'est la différence dure avec le `billing` web-saas). Redirection vers la page de remerciement.
4. **Webhook → commande créée UNE SEULE FOIS + stock décrémenté** — le webhook Stripe (`checkout.session.completed` / `payment_intent.succeeded`) se déclenche et **crée la commande** (source de vérité = Stripe), **exactement une**, **vérifiée à la source** — pas l'écran de remerciement : elle réapparaît dans « mes commandes » (si compte) **et** dans le back-office admin. Le **stock est décrémenté atomiquement** (la fiche produit reflète le nouveau stock). Commande absente, en double, ou stock inchangé = `FAIL` **bloquant**.
5. **Boucle fermée EC4 — non négociable, les DEUX côtés** (`_shared/boucles-fermees.md`, comme la contrepartie client↔gérant de web-saas) : l'**email de confirmation de commande** arrive **au client** dans la boîte sandbox (Inbucket/Mailpit en local, boîte de test du provider en staging — `_shared/safety-rails.md` §2) **ET** le **marchand est notifié** (nouvelle commande — email pro / notif admin). Réception **immédiate** (pas en déclenchant un cron à la main — même règle que la boucle web-saas). Vente muette d'un côté (email jamais reçu, marchand jamais prévenu) = `FAIL` **bloquant**.
6. **Back-office admin + « mes commandes »** — la commande est **visible** côté admin et **marquable expédiée** (`fulfilled`) ; le CRUD produits/stock fonctionne ; si comptes clients, « mes commandes » liste la commande du client. Back-office absent, commande invisible, ou statut non modifiable = `FAIL`.
7. **Légal de vente** — **CGV / mentions de vente adaptées à `jurisdiction`/`locale`** présentes et liées au footer (🚨 source du champ : `_shared/state-schema.md` §locale/jurisdiction) : dès qu'il y a **vente**, les CGV / Terms of Sale sont exigées **en plus** des mentions légales / Privacy, conditionnées par la juridiction — jamais « FR » en dur (une boutique EN avec Terms of Sale + Privacy = **conforme**). Absentes = `FAIL`.
8. **Verdict** — « boutique **qui vend pour de vrai et clôt sa boucle** » vs « **tunnel d'achat en carton** » : un inconnu achète **sans compte**, sa commande existe **une seule fois**, le stock bouge, **les deux parties** sont prévenues. Les **3 preuves DURES** (bloc dédié ci-dessous, P1/P2/P3) s'ajoutent obligatoirement : **une seule non prouvée = porte 14 fermée**.
...
#### Boucle de mesure — l'event d'activation émet réellement (jumeau analytics, conditionné par archétype)
L'action de valeur doit **émettre son event** au même titre qu'elle **notifie** — le `capture()` est posé au **même call-site** que le dispatch de notif (`skills/12-build/references/integration-pass.md` §Boucle de mesure). Quand PostHog est configuré en staging (`NEXT_PUBLIC_POSTHOG_KEY` présent), après le parcours d'activation, vérifie dans **PostHog live-events** que le funnel remonte : `user_signed_up` puis `activation_completed` (le moment magique du PRD), et que `identify()` a rattaché l'utilisateur. **Event attendu absent des live-events alors que l'action a réussi** = `capture()` non câblé au call-site → `FAIL` bloquant (funnel muet : 18-metrics lira du vide au 1er run — bug « AgencyDesk »). **Conditionné par archétype/type** : web-saas public/interne/perso = funnel signup→activation ; `landing` = `waitlist_joined` à l'inscription ; `automation` = **n/a funnel** → on vérifie que le run alimente **`run_log`** (AU2) + le healthcheck (AU3), pas PostHog ; `ecommerce` = **funnel e-commerce** `product_viewed` → `add_to_cart` → `checkout_started` → `order_completed` (panier moyen / taux de conversion / abandon de panier — **PAS** le funnel d'activation SaaS, `_shared/archetypes/ecommerce.md` §Pipeline). **Repli honnête** : PostHog non configuré → non bloquant, tracé (pas d'event simulé).
...
#### Les 3 preuves DURES ecommerce — P1 survente · P2 prix · P3 webhook (obligatoire dès `archetype = ecommerce`, bloquant)
> 🚨 **Trois pièges cassent une boutique en prod et NE SE VOIENT PAS au build** (`_shared/archetypes/ecommerce.md` §Pièges — l'équivalent e-commerce de l'idempotence 2-grains d'automation). `next build` / `tsc` ne voient ni une survente sur le dernier article, ni un prix client accepté tel quel, ni une commande dupliquée à la redelivery Stripe. Le faux-client les **provoque exprès** (comme il rejoue le déclencheur automation pour l'idempotence). **Aucune n'est un `CONCERNS`** : **une seule non prouvée = porte 14 FERMÉE** (rejeu de la porte 13, cran sécurité renforcé — `_shared/archetypes/ecommerce.md` §Critères d'acceptation).

1. **P1 — survente / course sur le stock (le pire)** — mets un produit à **stock = 1**. Lance **deux commandes concurrentes sur le dernier article** (deux sessions/onglets qui checkout le même produit quasi-simultanément). Attendu : **une seule réussit**, l'autre voit « épuisé » / est refusée ; le **stock ne descend jamais sous 0**. Les **deux** qui aboutissent (deux clients payés, un non livrable) = `FAIL` **bloquant** — c'est le `SELECT stock` puis `INSERT` applicatif (fenêtre de course) au lieu du **décrément conditionnel atomique**. Preuve : les deux issues (1 succès / 1 rupture) + le stock final **≥ 0** lu à la source.
2. **P2 — intégrité du prix (sécurité)** — soumets un panier au **prix/total altéré** : manipule le payload (onglet réseau / rejeu de requête) pour envoyer `price: 0.01` (ou un total minoré). Attendu : le **serveur rejette ou recalcule** depuis le catalogue à partir des **seuls `product_id` + quantités** — **jamais** le prix client ; Stripe débite le **prix serveur**. Un débit au prix altéré = `FAIL` **bloquant** (confiance au prix client). Preuve : le montant facturé = prix catalogue, **pas** `0.01`.
3. **P3 — idempotence du webhook (redelivery Stripe)** — **rejoue** le webhook de paiement (redelivery Stripe / renvoi du même event, **même `session_id` / `payment_intent`**). Attendu : **aucune commande en double** — contrainte unique + upsert `on conflict do nothing` ; la commande reste **unique** même après 2-3 redeliveries. Une 2ᵉ commande créée = `FAIL` **bloquant** (idempotence absente — le pendant e-commerce du « re-run ne double pas l'effet » d'automation). Preuve : `count(orders) == 1` pour ce paiement après re-livraison.

**RLS commandes — un client ne voit pas les commandes d'un autre** (le cloisonnement cross-tenant de « Cœur RLS-protégé », appliqué aux commandes) : si le produit propose des **comptes clients**, connecté sous le client A, tente de **voir** une commande du client B (ID deviné, liste « mes commandes » d'un autre) → attendu **refus / 0 ligne** (RLS : le client voit SES commandes, l'admin toutes — `_shared/archetypes/ecommerce.md` §Architecture). Une commande d'autrui visible = `FAIL` **bloquant**. En **checkout invité pur** (sans comptes), le test se réduit à : une commande n'est **pas énumérable** par un tiers (pas d'ID séquentiel devinable exposant les commandes d'autrui, pas de page de confirmation accessible sans jeton).

**Régression par preuve** *(cf. « Sur bug »)* : chaque piège attrapé génère son test de non-régression — P1 → test de **concurrence** (2 checkouts parallèles sur `stock=1` → une seule commande + stock ≥ 0) ; P2 → test de **payload altéré** (`price:0.01` → montant serveur facturé) ; P3 → test de **rejeu de webhook** (même `session_id` × N → 1 commande).
...
| **Achat prouvé à l'écran** *(ecommerce)* | « Merci pour votre commande » à l'écran → `PASS`, sans vérifier commande + stock + emails à la source | La page `/merci` n'est **pas** la preuve : exige la **commande à la source** (back-office admin + « mes commandes »), le **stock décrémenté**, et la **boucle EC4 des deux côtés** (email client + notif marchand en sandbox). Écran ≠ vente. |
| **Survente jamais provoquée** *(ecommerce)* | Un seul checkout joué sur un stock large → `PASS`, la course sur le dernier article jamais testée | Mets `stock=1`, lance **2 checkouts concurrents** : deux succès = `FAIL` **bloquant** (P1). Le décrément atomique ne se prouve **qu'en concurrence**, jamais en achat solo. |
| **Prix client accepté** *(ecommerce)* | Le total vient du client et personne ne l'a altéré pour tester | Rejoue la requête de checkout avec `price:0.01` : un débit ≠ prix catalogue = `FAIL` **bloquant** (P2). Ne fais **jamais** confiance au total envoyé par le client. |
| **Webhook jamais rejoué** *(ecommerce)* | La commande est créée une fois, la redelivery Stripe jamais simulée | **Rejoue** le même event (même `session_id`) : une 2ᵉ commande = `FAIL` **bloquant** (P3). Stripe redélivre **at-least-once** en prod — l'idempotence se prouve au rejeu, comme le re-run automation. |
