...
2. **Signup OTP / magic link** — crée le compte. Vérifie la **réception réelle** du code/lien (boîte de test sandbox : Inbucket/Mailpit en local, boîte de test du provider en staging — `safety-rails` §2) **et sa saisie**. Code qui n'arrive pas, expire trop vite, ou boucle de renvoi = `FAIL` bloquant. Signup **mot de passe seul** (sans OTP ni magic link) = `FAIL` (non conforme au standard produit).
