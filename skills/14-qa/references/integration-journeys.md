...
> 🚨 **La forme ci-dessous (landing → signup → onboarding → paiement → résultat) est celle de l'archétype `web-saas`.** Un **`landing`** n'a qu'un mini-parcours *visiteur → CTA/waitlist → confirmation* (aucune jonction auth/billing) ; un **`automation`** a un parcours *déclencheur → run → effet/idempotence → boucle fermée* (jonctions = *run → persistance*, *run → notification propriétaire* ; **pas d'UI**) ; un **`ecommerce`** a un parcours *catalogue → panier → checkout invité → paiement → webhook → commande/stock* (jonctions = *ajout panier → sous-total serveur*, *checkout → prix recalculé serveur*, *webhook paiement → commande unique + stock décrémenté + boucle client/marchand* ; **paiement one-shot, pas d'abonnement**). Renvoi : `fake-client-protocol.md` « Parcours #0 » + `_shared/archetypes/{landing,automation,ecommerce}.md`.
...
| **Ajout panier → sous-total** *(ecommerce)* | `product_id` + quantité → **sous-total recalculé serveur** | Total **envoyé par le client** et accepté tel quel (`price:0.01`) → prix falsifié (**piège P2**, `_shared/archetypes/ecommerce.md`) ; exige un recalcul serveur depuis le catalogue. |
| **Checkout → paiement one-shot** *(ecommerce)* | Montant serveur → Stripe `mode:payment` | `mode:subscription` au lieu de `payment` (abonnement au lieu d'achat) ; montant pris du panier client, pas recalculé depuis le catalogue. |
| **Webhook paiement → commande + stock** *(ecommerce)* | Event Stripe → **1 commande** (idempotent) + **décrément atomique** du stock | Commande créée à chaque redelivery → **doublons** (**piège P3**) ; `SELECT stock` puis `INSERT` → **survente** sur le dernier article (**piège P1**) ; commande créée **avant** le webhook (source de vérité ≠ Stripe). |

> **Calibrage par type** : les jonctions *billing/paiement* (abonnement) ne s'appliquent qu'aux produits **public + billing** ; en **interne/perso**, elles sont remplacées par la vérification « **signup anonyme refusé** » (accès privé réel). Les jonctions **ecommerce** (*ajout panier → sous-total serveur*, *checkout → paiement one-shot*, *webhook paiement → commande + stock*) s'appliquent dès `archetype = ecommerce` — **paiement one-shot, pas un abonnement**, et remplacent les jonctions billing web-saas. La jonction **boucle fermée**, elle, est **universelle** — dès qu'une contrepartie existe (client↔marchand inclus), elle vaut pour tous. **Route selon `skills/saas-factory/references/routing.md`** (ligne `14-qa`) — ne recopie pas la matrice.
...
- [ ] **Si `archetype = ecommerce`** : les **3 preuves DURES** provoquées exprès — **P1** survente (`stock=1` + 2 checkouts concurrents → 1 seule commande, stock ≥ 0), **P2** prix (`price:0.01` → montant serveur), **P3** webhook rejoué (même `session_id` → 1 commande) — **+ RLS commandes** (un client ne voit pas celles d'un autre). Une seule non prouvée = **porte 14 fermée** (`fake-client-protocol.md` §3 preuves DURES ecommerce).
...
| **Pièges ecommerce non provoqués** | Le parcours d'achat « passe » (achat solo OK) mais survente / prix altéré / webhook rejoué jamais testés | Si `archetype = ecommerce`, provoque **exprès** : **P1** (`stock=1`, 2 checkouts concurrents), **P2** (`price:0.01`), **P3** (webhook rejoué même `session_id`). Une seule non prouvée = **porte fermée** (`fake-client-protocol.md` §3 preuves DURES ecommerce). Ces bugs ne sortent **jamais** d'un achat nominal unique. |
