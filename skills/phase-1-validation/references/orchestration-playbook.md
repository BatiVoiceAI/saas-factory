...
| 2 | `02-market` | Teardown concurrents + review-mining + vérif adversariale | `idea-brief` | `research/market.md`, `research/confidence.md` | `startup-competitors` (waves 1→3) | — |
...
| 5 | `05-opportunity` | Synthèse 1→4 + verdict + décision | tous les précédents | `research/opportunity-brief.md`, `research/opportunity-summary.md` (+ `post-mortem.md` si No-Go) | — (overlay maison) | 🚪 **Go / Ajuster / No-Go** |
...
| Type | 02-market | 03-positioning | 04-demand-edge | 05-opportunity | Livrable de phase |
|---|---|---|---|---|---|
| **public** | ✅ complet | ✅ complet | ✅ complet | ✅ complet | brief + summary + porte Go/Ajuster/No-Go |
| **interne entreprise** | ⤳ **remplacé** par un **fit-check** (outils / process / contraintes sécu de la boîte) — pas de marché public | ⛔ sauté | ⛔ sauté (pas d'edge concurrentiel) | ✅ **décision d'utilité interne** | brief de fit + décision |
| **perso** | ⛔ sauté | ⛔ sauté | ⛔ sauté | ✅ **allégée** (« cet outil te sert à toi — on build ? ») | résumé court + décision build/non |
...
- **Au démarrage de la phase**, l'orchestrateur lit **une fois** : `references/conventions.md`, `_shared/lessons.md`, `_shared/safety-rails.md`. Ces règles priment sur le comportement par défaut et restent valables toute la phase.
