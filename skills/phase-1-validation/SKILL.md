...
2. **`02-market`** — teardown concurrents + review-mining, vérif adversariale (moteur `startup-competitors`). Écrit `research/market.md` (dont §Fiabilité & confiance, ex-`confidence.md` fusionné §5).
...
5. **`05-opportunity`** — synthèse 1→4 → `research/opportunity-brief.md` : le détaillé traçable **+** le bloc **§Décision en tête** (le **POURQUOI**, non-tech, auto-suffisant — fusion §5 : plus de `opportunity-summary.md` séparé). **Porte Go / Ajuster / Go-test / No-Go.**
...
```
   ...
   └──────────────┘              │               │                           │
        ▼                        ▼               ▼                           │
   ┌────────────────────────────────────────────────────────┐               │
   │ 05-opportunity — synthèse → brief (§Décision = POURQUOI)│               │
   └────────────────────────────────────────────────────────┘               │
        │                                                                    │
        ▼   🚪 PORTE Go / Ajuster / Go-test / No-Go                          │
     ┌────────┬──────────────┬───────────────┬─────────────┐                │
     │   Go   │   Ajuster    │    Go-test    │    No-Go    │                │
         ▼           ▼               ▼              ▼      └────────────────┘
  Phase 2   reboucle       landing+waitlist   post-mortem
 (produit)  étape faible   seuil pré-enreg.   5 lignes
            (gates.md)     → retour porte 5   + arrêt / archive
```
...
La décision se prend à l'étape 5 : l'expert présente le **bloc §Décision** (le POURQUOI, non-tech, en tête de l'`opportunity-brief`) + un verdict honnête, puis `AskUserQuestion`. **Go** → Phase 1 terminée, on passe au cadrage produit (Phase 2). **Ajuster** → reboucle sur l'étape faible (routage détaillé dans `05-opportunity` et matrice de retour arrière dans `references/state-and-gates.md`). **Go-test** *(public)* → ship une landing+waitlist à seuil pré-enregistré avant tout build, puis retour à la porte 5 avec la donnée réelle (détail : `skills/05-opportunity/references/go-test-playbook.md`). **No-Go** → post-mortem 5 lignes + arrêt propre. **Ne franchis jamais la porte sans décision explicite.** L'objectif : que l'utilisateur décide **en âme et conscience**, sur une base marché/fonctionnelle claire.
