...
> ✅ Périmètre : châssis `automation` + `ecommerce` (logique) LIVRÉS, `landing` assemblable clé-en-main du sous-ensemble web-saas, substrat `multi-org` (`org-tenancy`) LIVRÉ. On applique la route sur du buildable ; reste, pour un `landing`, la calibration amont 02/03/05 (signalée honnêtement).
...
4. **`multi-org` = public + substrat org** : landing/pricing/SEO **conservés** (type public), et 17 vérifie que le substrat org (Org, membres, invitations, switch, rôles) est présent avant publication — bloc **`org-tenancy` LIVRÉ** (web-saas).
...
- Les châssis `automation`/`ecommerce` sont livrés, `landing` s'assemble du sous-ensemble web-saas, `multi-org` (`org-tenancy`) est livré : appliquer la route est **buildable** ; reste, pour un `landing`, la calibration amont 02/03/05 (signalée).
