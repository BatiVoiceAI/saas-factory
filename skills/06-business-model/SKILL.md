...
> - **`ecommerce`** → suit le **contrat `web-saas`** (écrit `business-model.md` + `pricing.md` quand `type = public`, lit `market.md` pour les prix/concurrents de la boutique) MAIS la **monétisation = vente de produits en PAIEMENT ONE-SHOT**, **pas un abonnement SaaS** : le `pricing.md` porte des **prix produits / marge / panier** (jamais des paliers Starter/Pro/Team d'abonnement), et les **métriques clés = panier moyen + taux de conversion** (pas MRR / rétention d'abonnement). Ne saute rien — c'est un archétype public avec un vrai marché.
...
> **Garde archétype × type (lu dans `research/idea-brief.md` + `state.md`).** **`archetype = automation`** → mouvements 5-6 **sautés**, livrable = **`product/roi.md`** (rapport plein, §« Archétype `automation` »), pas de canvas ni de pricing. **`archetype = ecommerce`** → mouvements 5-6 **exécutés** (boutique publique) MAIS le pricing porte des **prix produits (paiement one-shot)** — jamais des paliers d'abonnement — et les **métriques = panier moyen / taux de conversion**. Sinon (`web-saas`), garde `type` : les mouvements 5-6 ne s'exécutent que si **`type = public`**. `interne` → saute-les et écris le **ROI interne** (§« Mode interne » ci-dessous) dans `business-model.md`. `perso` → l'étape 06 entière est **sautée** par routage (`routing.md`) : rien n'est produit ici (ni `business-model.md` ni `pricing.md`) ; le saut est tracé dans `state.md` par l'orchestrateur.
