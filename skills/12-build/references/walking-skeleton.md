...
```
**Gestes de fondation ecommerce** (écrivain unique, avant tout fan-out — châssis `_shared/blocks/ecommerce/` **LIVRÉ** : logique commerce + SQL + pièges P1/P2/P3, `verify:machine` + tests verts ; on **câble cette logique livrée** à l'**UI dérivée du socle web-saas** : réutilise `skeleton` / `ui-shell` / `notifications` / `legal`, **remplace le bloc `billing` abonnement par un checkout one-shot**) :
