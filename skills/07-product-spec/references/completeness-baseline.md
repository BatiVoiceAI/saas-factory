...
| **Socle du mauvais archétype** | Dashboard parasite sur une `automation` ; onboarding wizard sur une `landing` ; landing traitée en web-saas vide | Le socle est **conditionné par l'archétype** : injecter le socle **de l'archétype** (S1-S8 / LP1-LP4 / AU1-AU5), pas celui d'un autre — cf. `_shared/state-schema.md` §socle-par-archétype |
