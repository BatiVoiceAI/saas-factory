...
- `references/completeness-baseline.md` — le socle « produit complet » **conditionné par l'archétype** (Must d'office : web-saas → S1-S8 · landing → LP1-LP4 · automation → AU1-AU5 · ecommerce → EC1-EC5 ; renvoi `_shared/state-schema.md` §socle-par-archétype) + l'aha moment (étapes 1, 2 et 3). Le calibrage par **type** (public/interne/perso) s'ajoute par-dessus, il ne remplace pas l'archétype.
...
- **Complet, pas creux.** Le socle « produit complet » (`completeness-baseline.md`) est **Must d'office**, **CONDITIONNÉ PAR L'ARCHÉTYPE** (web-saas → S1-S8 · landing → LP1-LP4 · automation → AU1-AU5 · ecommerce → EC1-EC5 ; `_shared/state-schema.md` §socle-par-archétype), puis calibré par type (canal adapté) : un MVP « petit » reste un **produit fini** de son archétype, pas un squelette. **Ne jamais exiger onboarding+dashboard d'un landing, d'une automation headless ou d'un ecommerce** (dont le socle est EC1-EC5 — catalogue/panier/checkout/commandes/stock, checkout invité par défaut, pas S1-S8) (faux-négatif de porte — les portes de complétude sont par archétype, `routing.md`).
