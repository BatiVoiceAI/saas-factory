...
- **`og:locale` perdu sur une page qui override `openGraph`** (Next.js App Router) → Next **REMPLACE** l'objet `openGraph` d'un layout dès qu'une page en déclare un (pas de deep-merge des sous-objets). Une page qui pose son propre `openGraph` (ex. la **landing**, pour `og:type`/`og:url`/`twitter`) **perd** le `locale` hérité du `RootLayout` → `og:locale` disparaît sur la page la plus stratégique = le mode d'échec « socle SEO aveugle à la langue ». **Correctif** : ré-affirmer `locale: ogLocale` (source unique `lib/i18n`) dans **chaque** `openGraph` de page, jamais en dur. **Vérif livret QA** : `curl <page> | grep 'og:locale'` sur la landing ET une page héritée — les deux doivent le porter.
