...
- **Écrit :** **(a) le socle technique dans le code, committé sans gate** — `app/sitemap.ts`, `app/robots.ts`, metadata + canonical par route (sous `app/**`) + la section « SEO technique » du **livret QA** (`qa/test-booklet.md`, un check par élément du socle) ; **(b)** `seo/topic-cluster-map.md` (template) + `seo/plan.md` (template) + les **pages éditoriales dans le code** (chemin explicite : sous `app/` — ex. `app/(marketing)/**` : landing + une page par cluster, après gate humain) + `.saas-factory/state.md` (étape 16 faite).
