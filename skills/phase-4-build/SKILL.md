...
## À lire d'abord
`references/conventions.md` (persona l'org, déterminisme, contrat d'artefacts, calibrage par type, protocole de porte, moteurs vendorés).

> Discipline `_shared` : le **master** a déjà lu `_shared/*` (`validation-cascade.md` — niveaux, règles, budget —, `test-dossier.md`, `lessons.md`, `safety-rails.md`, …) au démarrage — ils **priment** et sont déjà en contexte. Ne les relis pas, ne les fais **pas relire** par chaque étape. Charge le détail d'une étape (ses `references/`) **au moment** où elle s'exécute. Détail : `references/orchestration-playbook.md`.
...
- **`references/orchestration-playbook.md`** — le **flux exécutable** : diagramme ASCII de la Phase 4, table séquence (étape → invoque → lit → écrit → porte), contrôle de réception anti-squelette, ouverture QA Analyst, gestion du **budget de cascade**, discipline « lire `_shared` une fois », valeur de l'orchestrateur.
...
## Calibrage (par type)
Le `type` (state.md, décidé Phase 1) calibre la **rigueur**, pas la présence des étapes : les 4 tournent toujours. Les **exigences par type** (gates 12/14 conditionnés — landing vs racine=login, test « signup anonyme refusé » —, persona du 15) vivent dans la matrice canonique : **route selon `skills/saas-factory/references/routing.md`**. En profondeur : `public` → cascade pleine (a11y + sécu strictes), budget large. `interne` → sécu/fonctionnel stricts, cran Designer orienté usage (pas brand), sponsor interne au 15. `perso` → cascade allégée (Designer/CEO au minimum utile), budget serré, 15 très court (l'humain = toi). Calibrage détaillé : `references/conventions.md`.
