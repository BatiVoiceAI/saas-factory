...
- **Modèle de base = Opus 4.8, mode qualité maximale (ultracode).** Le plugin opère de bout en bout sur le modèle le **plus capable**. **Chaque** sous-agent (`agents/`) tourne sur **Opus** (frontmatter `model: opus`) — on ne **descend jamais** en gamme « pour aller plus vite » ni « pour le coût » : le fondateur tranche **qualité > coût/vitesse**. Le fan-out multi-agents privilégie la **profondeur** et la **vérification adverse** (chaque validateur cherche à *réfuter*, cf. cascade de validation), pas le débit. Un modèle inférieur n'est envisageable que sur un geste **purement mécanique** **ET** **explicitement décidé** au cas par cas — **jamais le défaut**. Cohérent avec « on ne bâcle pas, on ne franchit pas une porte pour avancer ».
