...
## Continue
Signal présent (activation, rétention, premières conversions) → **itère** : retour étape 18 (mesure) → nouvelles pistes → build (Phase 4) → re-déploie. La boucle produit.
...
Si **transverse** (vaut pour tout projet futur) → **remonte-la dans `_shared/lessons.md`**. C'est le mécanisme qui rend SaaS Factory **meilleur à chaque cycle**. (Procédure détaillée : `memory-compound.md`.)
...
| Signal clair (activation OK, rétention qui plafonne > 0, 1res conversions) | **Continue** | repartir ét. 18 avec **budget d'itérations borné** (`safety-rails` §7) et 1-3 pistes |
...
- **Continuer** — itérer avec budget borné, 1-3 pistes reliées chacune à une métrique (retour ét. 18).
...
- [ ] Si **Continue** : budget d'itérations défini + 1-3 pistes reliées à une métrique.
