...
6. **La landing suit `_shared/landing-playbook.md`** — c'est le contrat, pas une inspiration :
   - **Structure = ARCHÉTYPE choisi (plus d'ordre imposé)** : sélectionne dans le **menu d'archétypes structurels** du playbook (Conversion-dense · Éditorial/narratif · Product-led · Trust local · Marketplace · Statement) **celui dont la forme sert le white space du métier et la direction visuelle recherchée** (mvt 1) — jamais un ordre par réflexe. L'**invariant minimum** du playbook (hero 5-sec + visuel réel · CTA répété · preuve honnête · pricing si vente · footer+légal · copy spécifique) est **non négociable** ; l'ordre/emphase/layout du reste suit l'archétype. **La silhouette de page choisie fait partie de la distinctiveness** (deux projets ≠ même silhouette). Applique les motifs de layout de la direction (hero asymétrique, bento, section de rupture…) au service de l'archétype.
