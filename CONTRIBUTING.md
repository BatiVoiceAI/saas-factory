...
## Périmètre — couverture des archétypes (tous couverts au châssis)
**Couverts** : **`web-saas`** + **`automation`** (châssis complets A-à-Z) + **`ecommerce`** (logique commerce + SQL + pièges P1/P2/P3 livré et testé — `_shared/blocks/ecommerce/` ; l'**UI se dérive du châssis web-saas**) + **`landing`** (**assemblage clé-en-main** du sous-ensemble web-saas : skeleton + `components/landing/*` + `waitlist` + notifications + légal, tous livrés `tsc`-verts) + le **substrat `org-tenancy`** (bloc web-saas `0006_org_tenancy.sql` + `lib/org/*` + org-switcher, RLS par org, `tsc`-vert). La **calibration amont** d'un `landing` (02/03/05 allégés) est **implémentée** (routing §Archétype landing + conditionnement `archetype=landing` dans `02-market` / `03-positioning` / `05-opportunity`). Seul résidu **universel** (pas propre au périmètre) : l'**épreuve run** (build SSR + E2E sur Supabase/env réels) se fait à la 1re instanciation, comme tout web-saas (`_shared/blocks/web-saas/MOAT-STATUS.md`). Dits explicitement dans `skills/saas-factory/references/routing.md` + `_shared/state-schema.md` — un run dit l'état réel, sans sur- ni sous-vendre.
