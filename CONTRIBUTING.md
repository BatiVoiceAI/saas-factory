...
`landing` (pas de châssis `_shared/blocks/landing/`) et le **substrat org/multi-org** sont **différés**, dits explicitement dans `skills/saas-factory/references/routing.md` + `_shared/state-schema.md`. Ne pas laisser un run bluffer dessus : il doit s'arrêter en le disant.
