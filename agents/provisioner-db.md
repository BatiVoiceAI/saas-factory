...
2. **Passwordless OTP/magic link** (le flux d'auth du châssis, cf. `blocks/web-saas` bloc `auth`) :
   ...
   - **Longueur du code — `mailer_otp_length = 6` (OBLIGATOIRE, invariant).** ⚠️ Le défaut Supabase est **8**. L'input OTP du châssis n'accepte que **6** cases (`blocks/web-saas/components/auth/auth-form.tsx` slice à 6 ; `lib/auth/actions.ts` valide `.length(6)`). **Sans ce PATCH, l'e-mail contient un code à 8 chiffres qui ne rentre pas dans l'input → la vérification échoue pour TOUS les utilisateurs.** Invariant à tenir : `mailer_otp_length` (Supabase) = nombre de cases de l'input OTP (châssis). Si l'un change, changer l'autre.
...
- **GET** `.../config/auth` d'abord → compare → **PATCH** les champs qui diffèrent : `smtp_host`, **`smtp_port` (=`'587'`, CHAÎNE — gotcha (a))**, `smtp_user`, `smtp_pass`, `smtp_admin_email`, `smtp_sender_name`, `smtp_max_frequency` (~`15`), `rate_limit_email_sent` (~`30`), `mailer_otp_exp`, **`mailer_otp_length` (=`6`, cf. invariant volet 2)**, `mailer_subjects_magic_link`, `mailer_templates_magic_link_content`, `mailer_subjects_confirmation`, `mailer_templates_confirmation_content`, `mailer_autoconfirm`, `site_url`, `uri_allow_list`. **⚠️ Les 6 champs `smtp_*` forment un bloc indivisible** : si l'un diffère, renvoie-les **tous** dans le même PATCH (un PATCH smtp partiel vide les autres → 500, gotcha (b)).
